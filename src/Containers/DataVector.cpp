...
namespace LLU {
	...
	DataVector::MContainer(const std::vector<std::string_view>& string_data, const Int8Array& validity) {
		...
		const auto* api = LibraryData::TabularAPI();
		Container result;
		checkAPICall(api->TabularColumn_newString(str_count, characters.release(), offsets.release(), validity.getContainer(), &result));
		reset(result);
	}

	DataVector::MContainer(GenericNumericArray&& array, mint elem_count, UniquePtr<mint[]>&& offsets, const Int8Array& validity) {
		const auto* api = LibraryData::TabularAPI();
