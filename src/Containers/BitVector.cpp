...
namespace LLU {
	BitVector::BitVector(mint length, bool setQ) {
		const auto error = LibraryData::DataVectorAPI()->BitVector_new(length, false, &bv);
		if (error) {
			// TODO: throw
		}
	}
	...
	BitVector::BitVector(BitVector&& other) noexcept : bv{other.bv} {
		other.bv = nullptr;
	}
