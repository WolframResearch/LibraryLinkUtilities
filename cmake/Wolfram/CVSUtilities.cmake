...
function(parse_components_config)
	...
	if (LIBRARY_CONF MATCHES .*\\.json)
		...
		foreach(INDEX RANGE ${_MAX_LIBRARY_INDEX})
			...
			# ${_LIB_NAME}_VERSION can be set ahead of time in case there are multpile versions of the library to choose from
			set(_REQUEST_${_LIB_NAME}_VERSION "${${_LIB_NAME}_VERSION}")
			if("${${_LIB_NAME}_VERSION}" AND NOT _REQUEST_${_LIB_NAME}_VERSION)
				set(_REQUEST_${_LIB_NAME}_VERSION "${${_LIB_NAME}_VERSION}")
			endif()
			if(_REQUEST_${_LIB_NAME}_VERSION AND (NOT _REQUEST_${_LIB_NAME}_VERSION STREQUAL ${_LIB_VERSION}))
				continue()
			endif()
			list(APPEND _LIB_NAME_LIST ${_LIB_NAME})
		...
		endforeach()
		foreach(_LIB_NAME IN LISTS _LIB_NAME_LIST)
			unset(_REQUEST_${_LIB_NAME}_VERSION)
		endforeach()
	else()
